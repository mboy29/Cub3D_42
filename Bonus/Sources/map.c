...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/07 13:45:02 by mboy              #+#    #+#             */
/*   Updated: 2021/06/07 13:45:13 by mboy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_struct(t_cub *cub)
{
	...
	cub->ray->cutX = 25;
	cub->ray->cutY = 25 + cub->game->messH;
	return (0);
}
...
void	map_sqt(t_cub *cub, int i, int j)
{
	...
	else if (cub->parse->map[i][j] == '2' || cub->parse->map[i][j]
			== '3' || cub->parse->map[i][j] == '4')
		gen_sqr(cub->ray, cub->ray->sqr_side, 0x00402E26, 0x00271C17);
	else if (cub->parse->map[i][j] == '5')
		gen_sqr(cub->ray, cub->ray->sqr_side, 0x00FAAB1C, 0x00271C17);
}
