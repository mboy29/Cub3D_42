...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/07 13:46:27 by mboy              #+#    #+#             */
/*   Updated: 2021/06/07 13:46:28 by mboy             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_line(char *line, t_cub *cub)
{
	...
	while (line[++i])
	{
		...
		if (parse_line_select(line, i, cub) == -1)
			return (-1);
	}
	return (0);
}
...
int	parse(char *str, t_cub *cub)
{
	...
	{
		if (ft_strlen(line) > 0)
			if (ret == -1 || parse_line(line, cub) == -1)
				return (ex(cub, 3, str));
		free(line);
		ret = get_next_line(fd, &line);
	}
	if (parse_line(line, cub) == -1 || cub->parse->letter != TEXT)
		return (ex(cub, 3, str));
	close (fd);
	free(line);
	if (parse_tab(str, cub) == -1)
		return (ex(cub, 3, str));
	return (0);
}
